...
class LoginSerializer(serializers.Serializer):
    ...
    def _validate_username_phone(self,phone, password):
        if phone and password:
            user = self.authenticate(phone=phone, password=password)
        # elif username and password:
        #     user = self.authenticate(username=username, password=password)
        else:
            msg = _('Must include either "username" or "phone" and "password".')
            raise exceptions.ValidationError(msg)

        return user

    def get_auth_user_using_allauth(self,phone, password):
        from allauth.account import app_settings

        # Authentication through phone
        if app_settings.AUTHENTICATION_METHOD == app_settings.AuthenticationMethod.PHONE:
            return self._validate_email(phone, password)

        # Authentication through username
        if app_settings.AUTHENTICATION_METHOD == app_settings.AuthenticationMethod.USERNAME:
            return self._validate_username(phone, password)

        # Authentication through either username or phone
        return self._validate_username_phone(phone, password)

    def get_auth_user_using_orm(self,phone, password):
        # if phone:
        #     try:
        #         username = UserModel.objects.get(phone__iexact=phone).get_username()
        #     except UserModel.DoesNotExist:
        #         pass

        # if username:
        #     return self._validate_username_phone(username, '', password)
    ...
    def get_auth_user(self,phone, password):
        ...
        if 'allauth' in settings.INSTALLED_APPS:
            ...
            try:
                return self.get_auth_user_using_allauth(phone, password)
            except url_exceptions.NoReverseMatch:
                msg = _('Unable to log in with provided credentials.')
                raise exceptions.ValidationError(msg)
        return self.get_auth_user_using_orm(phone, password)
    ...
    # @staticmethod
    # def validate_phone_verification_status(user):
    #     from allauth.account import app_settings
    #     if (
    #         app_settings.PHONE_VERIFICATION == app_settings.EmailVerificationMethod.MANDATORY
